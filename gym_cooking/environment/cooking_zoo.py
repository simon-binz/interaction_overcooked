...
COST_FOR_WRONG_DELIVERY = -50.0


def env(level, num_agents, record, max_steps, recipes, obs_spaces, max_rounds, respawn_at_same_locations,
...
class CookingEnvironment(AECEnv):
        ...
        numeric_obs_space = {'symbolic_observation': gym.spaces.Box(low=0, high=10,
                                                                    shape=(self.world.width, self.world.height,
                                                                           self.graph_representation_length + len(
                                                                               self.world.agents)), dtype=np.int32),
    ...
    def compute_rewards(self):
        ...
        #I think this is a leftover from training for the correct delivery, not needed with pretrained agent/humans
        #else:
        #
