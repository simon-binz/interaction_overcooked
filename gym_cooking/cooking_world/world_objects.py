...
class Intentions():
    def __init__(self, intentionList, start):
        self.value = start
        assert(start >= 0 and start < len(intentionList))
        self.intentions = intentionList
        self.intention = self.intentions[self.value]
    def moveIntention(self, direction):
        if (direction == "up" and self.value > 0):
            self.value = self.value -1
            #print(self.value)
        if (direction == "down" and self.value < len(self.intentions) -1):
            self.value = self.value +1
            #print(self.value)
    def changeIntention(self):
        self.intention = self.intentions[self.value]
        print(self.intention.intention)
...
class IntentionButton(Button): #ToDo finish
...
class AgentButton(Button): #ToDo finish
